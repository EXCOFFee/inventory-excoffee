...
### [ ] P0-1 · Eliminar la race condition en el registro de movimientos
...
- [ ] El test existente de "insufficient stock" sigue verde.
- [ ] Nuevo test de **concurrencia**: dos `create` OUT simultáneos sobre el mismo producto
      con stock para solo uno → exactamente uno tiene éxito, el otro lanza 400, el stock final
      nunca es negativo.
- [ ] `stockAfter` coincide con el stock real en DB tras la operación.
- [ ] `pnpm test`, `pnpm lint`, `pnpm build` verdes.

---

### [ ] P0-2 · Exigir 2FA en el login (flujo de dos pasos)
...
Criterios de aceptación:
- [ ] Test: usuario sin 2FA → `login` devuelve `access_token` (sin regresión).
- [ ] Test: usuario con 2FA → `login` devuelve `requires2FA: true` y **no** `access_token`.
- [ ] Test: `/2fa/login` con código válido → `access_token`; con código inválido → 400;
      con `twoFactorToken` inválido/expirado → 401.
- [ ] Swagger (`/api/docs`) muestra ambos endpoints correctamente.
- [ ] `pnpm test`, `pnpm lint`, `pnpm build` verdes.
...
### [ ] P0-3 · Reparar el login web (mismatch de contrato `access_token`)
...
Criterios de aceptación:
- [ ] Test del frontend: login mock con `{ access_token }` → el token guardado NO es `undefined`.
- [ ] Login manual en la web funciona de extremo a extremo (se entra al dashboard).
- [ ] Mobile sigue logueando sin el fallback.
...
### [ ] P1-LINT · Reparar el lint roto (ESLint v9 sin flat config)
**Ref:** SDD H-14
...
Pasos:
- [ ] Crear `backend/eslint.config.js` (flat config) coherente con TypeScript + NestJS.
- [ ] Correr `pnpm lint` con la config nueva.
- [ ] Reportar el número de violaciones encontradas (**no** corregirlas todas a ciegas).
- [ ] Si son pocas (criterio sugerido: menos de ~15), corregirlas ahora mismo. Si son muchas,
      dejarlas como ítem aparte explícito (avisar antes de inventar una tarea nueva) y seguir.

Criterios de aceptación:
- [ ] `pnpm lint` ya no falla por configuración ausente.
- [ ] El número de violaciones restantes (si las hay) está documentado y comunicado.
- [ ] `pnpm build` sigue verde (la config nueva no rompe nada).
